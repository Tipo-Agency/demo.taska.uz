...
// Simple Login Component - Демо режим: просто кнопка входа
const LoginView = ({ users, onLogin }: { users: any[], onLogin: (u: any) => void }) => {
    const handleLogin = () => {
        // Автоматически входим как первый пользователь (admin)
    ...
    return (
        <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-[#121212] dark:to-[#1a1a1a] px-4">
            <div className="w-full max-w-md">
                ...
                {/* Simple Login Button */}
                <div className="bg-white dark:bg-[#252525] p-8 rounded-2xl shadow-xl border border-gray-200 dark:border-[#333]">
                    <button 
                        onClick={handleLogin}
                        className="w-full bg-[#2C7E20] hover:bg-[#24661a] text-white font-semibold py-4 rounded-lg transition-colors shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-[#2C7E20] focus:ring-offset-2 text-lg"
                    >
                        Войти в систему
                    </button>
                    <p className="text-xs text-center text-gray-500 dark:text-gray-400 mt-4">Демо-версия для ознакомления</p>
                </div>
            </div>
        </div>
    );
